...
class TestBitbucketServerService {
    ...
    @test async test_canInstallAutomatedPrebuilds_unauthorized() {
        const result = await this.service.canInstallAutomatedPrebuilds(this.user, "https://bitbucket.gitpod-self-hosted.com/users/jldec/repos/test-repo")
        expect(result).to.be.false;
    }

    @test async test_canInstallAutomatedPrebuilds_ok() {
        const result = await this.service.canInstallAutomatedPrebuilds(this.user, "https://bitbucket.gitpod-self-hosted.com/projects/FOO/repos/repo123")
        expect(result).to.be.true;
    }

    @test.only async test_installAutomatedPrebuilds_ok() {
        try {
            await this.service.installAutomatedPrebuilds(this.user, "https://bitbucket.gitpod-self-hosted.com/projects/FOO/repos/repo123");
        } catch (error) {
            expect.fail(error)
        }
    }
